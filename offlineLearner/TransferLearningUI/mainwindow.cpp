...
MainWindow::MainWindow(QWidget* parent)
    ...
    , mDatasetColum(11)
    , mDatasetRow(129994)
    , mValidationsetColum(11)
    , mValidationsetRow(129994)
    , mInputDimension_Sim2RealNet(10)
    , mOutputDimension_Sim2RealNet(1)
    , mEpochs(DEFAULT_EPOCHS)
    , mCurrentEpoch(0)
    , mEstimateIndex(0)
    , mEstimateRange(129994)
    ...
    , mGroundTruthValue(0.0)
{
    ...
    Sim2Realnet = std::make_shared<Sim2RealNet>(mInputDimension_Sim2RealNet, mOutputDimension_Sim2RealNet);
...
{
    ui->LE_DATASET_LOAD_PATH->setText("GRFDatasetsTransferLearning_real.csv");
    ...
    ui->LE_MODEL_SAVE_PATH->setText("Model_Sim2Real.pt");
    ui->LE_MODEL_LOAD_PATH->setText("Model_Sim2Real.pt");
}
...
//    std::cout<<"[DATASET LOADER] Datasets"<<mDataSet<<std::endl;
}
...
void MainWindow::doValidation(int* range)
{
    ...
    auto inputs = torch::randn({ 1, mInputDimension_Sim2RealNet });
    ...
    {
        for( int j = 0 ; j < mInputDimension_Sim2RealNet ; j++)
        {
            inputTensorAccessor.data()[j] = mDataSet.data()[i].data()[j].item<float>();
        }
        mGroundTruthValue = mDataSet.data()[i].data()[mInputDimension_Sim2RealNet].item<float>();
        mEstimateIndex = i;
        mEstimatedValue = Sim2Realnet->forward(inputs).item<float>();
...
void MainWindow::on_BT_LEARNING_clicked()
{
    ...
    torch::optim::Adam optimizer(Sim2Realnet->parameters(), torch::optim::AdamOptions(mLearningRate));
    auto inputs = torch::randn({ mBatchSize, mInputDimension_Sim2RealNet });
    auto target = torch::randn({ mBatchSize, mOutputDimension_Sim2RealNet });
    ...
    for (std::size_t epoch = 1; epoch <= mEpochs; epoch++)
    {
        ...
        for (auto& batch : *dataLoader)
        {
            ...
            for (int64_t i = 0; i < batch.size(); ++i)
            {
                for (int j = 0; j < 10 ; j++)
                {
                    inputTensorAccessor.data()[i * 10 + j] = batch.data()[i].data[j].item<float>();
                }
                targetTensorAccessor.data()[i] = batch.data()[i].data[mInputDimension_Sim2RealNet].item<float>();
            }
            auto out = Sim2Realnet->forward(inputs);
...
void MainWindow::on_BT_MODEL_SAVE_clicked()
{
    ...
    torch::save(Sim2Realnet, modelPath);
...
void MainWindow::on_BT_MODEL_LOAD_clicked()
{
    ...
    try
    {
        ...
        torch::load(Sim2Realnet, modelPath);
