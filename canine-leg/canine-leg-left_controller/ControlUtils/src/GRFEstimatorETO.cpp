...
void GRFEstimatorETO::updateBeta() {
    if (mbIsFirstRun) {
        mMotorVelPrev = 0;
        mMotorPosPrev = sharedMemory->motorPosition[1];
        mbIsFirstRun = false;

    }
    mBeta = mCoriMat * mMotorVelPrev - (0.5 * mMass2 * mLink2 * mLink2 * sin(mMotorPosPrev / 2.0)) * mMotorVelPrev;
    mMotorPosPrev = sharedMemory->motorPosition[1];
    mMotorVelPrev = sharedMemory->motorVelocity[1];
